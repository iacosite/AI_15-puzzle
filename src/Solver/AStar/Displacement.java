...
import State.mStep;
/***
 * The problem is solved by applying a A* Search combined with the displacement distance to the search tree
 ...
 *
 */
public class Displacement extends Solver{

	private List<CostStep> queue;

	public Displacement(State initial) {
		super("A* Displacement Distance", initial);
	...
	public long Solve() {
		...
		cStep = new CostStep(Arrays.asList(mStep.NOP), initial.evalWithDisplacement(Arrays.asList(mStep.NOP)));
		queue.add(cStep);
		if (initial.isSolvable()) {
			while (!goalFound) {
				...
				if (cStep.getEval().isValid() && visited.add(cStep.getEval().getHash())) {
					...
					if (!cStep.getEval().isOptimal()) {
						...
						tmp.add(mStep.Up);
						queue.add(new CostStep(tmp, initial.evalWithDisplacement(tmp)));
						tmp = new LinkedList<>(cStep.getStep());
						tmp.add(mStep.Down);
						queue.add(new CostStep(tmp, initial.evalWithDisplacement(tmp)));
						tmp = new LinkedList<>(cStep.getStep());
	...
	public long SolveWithGoals() {
		...
		tmp = new LinkedList<>();
		tmp.add(mStep.NOP);
		cStep = new CostStep(tmp, initial.evalGoalWithDisplacement(tmp, 0));
		queue.add(cStep);
		curState = new State(initial);

		/*
		 * Our objective is to divide the broader bigger problem is sub goals. Each goal
		 * should be easy enough to solve in a reasonable amount of time
		 */
		for (int goal : goals) {
			goalFound = false;
				...
				if (cStep.getEval().isValid()) {
					if (cStep.getEval().isOptimal()) {
						...
						if (visited.add(cStep.getEval().getHash())) {
							...
							tmp = new LinkedList<>(cStep.getStep());
							tmp.add(mStep.Up);
							queue.add(new CostStep(tmp, curState.evalGoalWithDisplacement(tmp, goal)));
							tmp = new LinkedList<>(cStep.getStep());
							tmp.add(mStep.Down);
							queue.add(new CostStep(tmp, curState.evalGoalWithDisplacement(tmp, goal)));
							tmp = new LinkedList<>(cStep.getStep());
			...
			tmp = new LinkedList<>();
			tmp.add(mStep.NOP);
			cStep = new CostStep(tmp, curState.evalGoalWithDisplacement(tmp, goal+1));
			queue.add(cStep);
